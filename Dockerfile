...
#   jsx_server  — `node server.mjs`, the pages server + API. Pages are NOT in the image;
#                 they are bind-mounted at /pages from the host.
...
ENV NODE_ENV=production PORT=3000 PAGES_DIR=/pages DISABLE_AUTOUPDATER=1
EXPOSE 3000
CMD ["node", "server.mjs"]
