...
5. When he agrees with the targets (or after you adjusted one with `api PUT …/target`), run
   `hoy --guardar` so the dashboard cards show them. Then nothing else: he sends sets as he
   goes; log them per the `registrar` skill.
