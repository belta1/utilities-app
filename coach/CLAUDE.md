...
| Command | Use |
|---|---|
| `hoy [YYYY-MM-DD] [--day martes] [--guardar] [--limpiar] [--json]` | Today's plan day: last 3 sessions per exercise (with RIR when logged), e1RM trend, how regularly the movement is actually trained, what's logged so far, and a rebalanced target — load, reps/seconds and sometimes the number of sets — each with the rule that produced it. **Run this before coaching a session.** It also prints `cobertura semana` (empuje/halar/pierna/cardio done vs owed by today) and, on a **light** day (recuperacion/descanso/cardio/core opcional) when the week is short, a **⚑ HOY SUGERIDO** that substitutes the missed session — it rebalances *that* day instead. `--guardar` writes the targets *and* the `plan:` lines *and* the recommendation (so the dashboard shows "HOY SUGERIDO"). `--limpiar` deletes today's recommendation (dashboard reverts to the plan day). The weekly plan is never edited. |
...
| `api GET /api/recommendation?date=<fecha>` · `api PUT /api/recommendation '{"recommended_on":"…","plan_key":"lunes","source_key":"sabado","reason":"…"}'` · `api DELETE /api/recommendation/<fecha>` | The date-scoped "HOY SUGERIDO" the dashboard renders. `hoy --guardar` / `--limpiar` normally manage it; write one yourself only to override the substitution. `plan_key` must be a real plan day. |
